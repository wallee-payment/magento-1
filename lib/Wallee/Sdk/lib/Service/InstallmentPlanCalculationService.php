...
class InstallmentPlanCalculationService {
	 ...
	 * @param int $transactionId The transaction for which the plans should be calculated for. (required)
	...
	public function installmentPlanCalculationCalculatePlansPostWithHttpInfo($spaceId, $transactionId) {
		...
		try {
			...
			switch ($e->getCode()) {
				case 200:
					$responseObject = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Wallee\Sdk\Model\InstallmentCalculatedPlan[]', $e->getResponseHeaders());
