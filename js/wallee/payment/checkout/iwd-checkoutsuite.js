			...
			if (typeof(result.status) !== 'undefined') {
				if (result.status) {
					if (this.getPaymentMethod(Singleton.get(PaymentMethod).getPaymentMethodCode()).handler) {
						...
						location.href = MageWallee.Checkout.paymentPageUrl + '&paymentMethodConfigurationId=' + this.getPaymentMethod(Singleton.get(PaymentMethod).getPaymentMethodCode()).configurationId;
					}
				} else {
